...
class Settings(BaseSettings):
    ...
    GITHUB_PAGES_BASE: Optional[str] = None  # computed later
...
logger.handlers = []  # Reset any inherited handlers
logger.addHandler(console_handler)
logger.addHandler(file_handler)
logger.propagate = False

def flush_logs():
    """Forces flushing of log handlers."""
    try:
        sys.stdout.flush()
        sys.stderr.flush()
...
class Attachment(BaseModel):
    name: str
    url: str  # expected data URI (data:...;base64,...) or http(s) link
...
# -------------------------
app = FastAPI(
    title="Automated Task Receiver & Processor (Merged Robust)",
...
# For status reporting (stores last received)
...
# -------------------------
# Attachment helpers (both data URI and external URLs)
# -------------------------
...
def data_uri_to_gemini_part(data_uri: str) -> Optional[dict]:
    ...
    """
    if not data_uri or not data_uri.startswith("data:"):
        logger.debug("Invalid Data URI provided to data_uri_to_gemini_part.")
        return None

    try:
        match = re.search(r"data:(?P<mime_type>[^;]+);base64,(?P<base64_data>.*)", data_uri, re.IGNORECASE)
        if not match:
            logger.warning("Could not parse MIME type or base64 data from Data URI.")
            return None
        ...
        logger.exception(f"Error building gemini part from data URI: {e}")
        return None

async def attachment_to_gemini_part(attachment_url: str) -> Optional[dict]:
    """
    Convert an attachment URL (data URI or external URL) into a Gemini 'inlineData' part.
    Supports data URIs and http(s) image URLs.
    """
    ...
    if attachment_url.startswith(("http://", "https://")):
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                if not mime_type.startswith("image/"):
                    logger.info(f"Skipping non-image Content-Type from URL: {mime_type}")
                    return None
                b64 = base64.b64encode(response.content).decode("utf-8")
                return {"inlineData": {"data": b64, "mimeType": mime_type}}
        except Exception as e:
            logger.warning(f"Failed to fetch/encode attachment URL {attachment_url}: {e}")
            return None
    return None
...
async def save_generated_files_locally(task_id: str, files: dict) -> str:
    """Saves the structured LLM output (index.html, README.md, LICENSE) locally."""
...
async def save_attachments_locally(task_dir: str, attachments: List[Attachment]) -> List[str]:
    """Saves non-LLM generated files (attachments) locally."""
    saved_files = []
        ...
        for attachment in attachments:
            ...
            try:
                ...
                if file_bytes is None:
                    logger.warning(f"No content found or could not parse for attachment: {filename}")
                    continue
                ...
                logger.exception(f"HTTP Error fetching attachment {filename}: {e.response.status_code}")
            except Exception as e:
...
async def setup_local_repo(local_path: str, repo_name: str, repo_url_auth: str, repo_url_http: str, round_index: int) -> git.Repo:
    ...
    async with httpx.AsyncClient(timeout=45) as client:
        try:
            ...
            logger.exception(f"GitHub API call failed during repository setup: {e.response.status_code} {getattr(e.response, 'text', '')}")
            raise
...
async def commit_and_publish(repo: git.Repo, task_id: str, round_index: int, repo_name: str) -> dict:
    """Commits files, pushes to GitHub, and ensures GitHub Pages is enabled."""
    github_username = settings.GITHUB_USERNAME
    github_token = settings.GITHUB_TOKEN
    headers = {
        "Authorization": f"token {github_token}",
            ...
            for attempt in range(pages_max_retries):
                try:
                    ...
                    if e.response.status_code == 422 and "main branch must exist" in text and attempt < pages_max_retries - 1:
                        ...
                        continue
                    logger.exception(f"Failed to configure GitHub Pages: {e.response.status_code} {getattr(e.response, 'text', '')}")
                    raise
...
# -------------------------
# Gemini / LLM helpers (hybrid strategy C)
# -------------------------
async def call_gemini_api(contents: list, system_prompt: str, response_schema: dict, max_retries: int = 3, timeout: int = 60) -> dict:
    """
    Generic low-level Gemini API call, returns parsed JSON object according to schema.
    """
    payload = {
        "contents": contents,
...
async def llm_generate_initial_and_refine(prompt: str, task_id: str, image_parts: list, system_prompt: str, response_schema: dict) -> dict:
    """
    Curren-style two-step approach: initial draft -> critique/refinement (used primarily for R1).
    """
    ...
    # Initial generation
    draft = await call_gemini_api(contents=contents, system_prompt=system_prompt, response_schema=response_schema, max_retries=4, timeout=90)

    # Refinement/critique step: pass draft index.html for debugging
    refinement_system_prompt = (
        "You are an expert QA Engineer and Debugger. Your task is to review the provided 'index.html' code "
        "and immediately fix any JavaScript errors, broken event handlers, or logic flaws in the application state management. "
        "Return the full JSON object with keys 'index.html', 'README.md', and 'LICENSE'."
    )

    code_for_critique = (
        "--- CODE FROM DRAFT STEP FOR CRITIQUE AND FIX ---\n\n"
        f"Filename: index.html\n\n{draft.get('index.html', '')}\n\n"
        "--- END OF CODE TO CRITIQUE ---\n"
    )

    critique_prompt = (
        "Please debug and fix the provided index.html. Ensure all event handlers are attached and the app runs without JS errors. "
        f"Goal/Brief: {prompt}\n\n{code_for_critique}"
    )

    critique_contents = [{"parts": [{"text": critique_prompt}]}]

    try:
        refined = await call_gemini_api(contents=critique_contents, system_prompt=refinement_system_prompt, response_schema=response_schema, max_retries=3, timeout=90)
    except Exception:
        # If refinement fails for whatever reason, fallback to draft
        logger.warning("[LLM_REFINE] Refinement failed; falling back to initial draft.")
        refined = {}

    # Merge ensuring keys exist
    final = {
        "index.html": refined.get("index.html") or draft.get("index.html", ""),
        "README.md": refined.get("README.md") or draft.get("README.md", "Generated README Placeholder."),
        "LICENSE": refined.get("LICENSE") or draft.get("LICENSE", "MIT License Placeholder.")
    }
    return final
...
async def llm_round2_surgical_update(prompt: str, task_id: str, image_parts: list, system_prompt: str, response_schema: dict, existing_files: dict) -> dict:
    ...
    """
    # For R2 we explicitly include existing files in the content for context
    existing_index = existing_files.get("index.html", "")
    existing_readme = existing_files.get("README.md", "")
    existing_license = existing_files.get("LICENSE", "")

    r2_prompt = (
        f"UPDATE INSTRUCTION (ROUND 2+): Apply only the requested change(s) from the brief to the provided existing project. "
        "You MUST be surgical and preserve core functionality. Return a full JSON with keys 'index.html', 'README.md', 'LICENSE'.\n\n"
        f"NEW BRIEF: {prompt}\n\n"
        "--- EXISTING index.html ---\n"
        f"{existing_index}\n"
        "--- END EXISTING index.html ---\n\n"
        "For README.md and LICENSE, copy their existing contents verbatim unless absolutely required to change.\n\n"
        f"EXISTING README.md:\n{existing_readme}\n\nEXISTING LICENSE:\n{existing_license}\n\n"
        "Return the FULL UPDATED FILES in JSON."
    )

    contents = [{"parts": [{"text": r2_prompt}]}]
    # include images if any (images not strictly required for code updates)
    if image_parts:
        contents[0]["parts"] = image_parts + contents[0]["parts"]

    try:
        updated = await call_gemini_api(contents=contents, system_prompt=system_prompt, response_schema=response_schema, max_retries=4, timeout=90)
    except Exception as e:
        logger.exception(f"[LLM_R2] Surgical update failed; error: {e}")
        # if LLM fails, fallback to returning existing files (no-op)
        updated = {
            "index.html": existing_index or "<!-- existing index.html returned due to LLM failure -->",
            "README.md": existing_readme or "Existing README preserved due to LLM failure.",
            "LICENSE": existing_license or "Existing LICENSE preserved due to LLM failure."
        }
    # Ensure keys exist
    updated['README.md'] = updated.get('README.md') or existing_readme or "README placeholder"
    updated['LICENSE'] = updated.get('LICENSE') or existing_license or "LICENSE placeholder"
    updated['index.html'] = updated.get('index.html') or existing_index or "<!-- index.html placeholder -->"
    return updated

async def call_llm_for_code_with_hybrid(task_data: TaskRequest, initial_prompt: str, image_parts: list, existing_files_context: Optional[dict] = None) -> dict:
    """
    Strategy C: Curren-style for Round 1 (generate + refine), Base-style for Round 2 (surgical update).
    """
    task_id = task_data.task
    round_index = task_data.round

    # Shared system prompt baseline
    system_prompt_base = (
    ...
    if round_index == 1:
        logger.info("[LLM-HYBRID] Running Curren-style generation (R1).")
        # Curren-style initial + critique refinement
        return await llm_generate_initial_and_refine(
            prompt=initial_prompt,
            task_id=f"{task_id}-r1",
            image_parts=image_parts,
            system_prompt=system_prompt_base,
            response_schema=LLM_RESPONSE_SCHEMA
        )
    else:
        logger.info("[LLM-HYBRID] Running surgical, Base-style update (R2+).")
        existing_files = existing_files_context or {"index.html":"", "README.md":"", "LICENSE":""}
        return await llm_round2_surgical_update(
            prompt=initial_prompt,
            task_id=f"{task_id}-r{round_index}",
            image_parts=image_parts,
            system_prompt=system_prompt_base,
            response_schema=LLM_RESPONSE_SCHEMA,
...
async def notify_evaluation_server(evaluation_url: str, email: str, task_id: str, round_index: int, nonce: str, repo_url: str, commit_sha: str, pages_url: str) -> bool:
    """Notifies the external evaluation server of the successful deployment."""
    payload = {
        "email": email,
        "task": task_id,
        "round": round_index,
...
async def receive_task(task_data: TaskRequest, request: Request):
    ...
    # Store last received for /status
    last_received_task = {
        "task": task_data.task,
        "email": task_data.email,
    ...
    # Create background asyncio task (non-blocking)
...
async def get_status():
    ...
    if last_received_task:
        # Filter completed tasks from the list for accurate running count
